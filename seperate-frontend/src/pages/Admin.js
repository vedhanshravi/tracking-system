...
function Admin() {
  ...
  const helpStatusCounts = {
    All: helpIssues.length,
    Open: helpIssues.filter((issue) => issue.status === "Open").length,
    "In progress": helpIssues.filter((issue) => issue.status === "In progress").length,
    Resolved: helpIssues.filter((issue) => issue.status === "Resolved").length,
    Cancelled: helpIssues.filter((issue) => issue.status === "Cancelled").length,
  };
          ...
          <div style={{ marginBottom: "16px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {['All', 'Pending', 'Approved'].map((status) => (
              <button
                key={status}
                type="button"
                onClick={() => setDocumentFilterStatus(status)}
                style={{
                  padding: "8px 14px",
                  borderRadius: 4,
                  border: documentFilterStatus === status ? "1px solid #007bff" : "1px solid #ccc",
                  background: documentFilterStatus === status ? "#007bff" : "#f8f9fa",
                  color: documentFilterStatus === status ? "white" : "black",
                  cursor: "pointer",
                }}
              >
                {status}
            ...
            {['All', 'Open', 'In progress', 'Resolved', 'Cancelled'].map((status) => (
              <button
                ...
                style={{
                  padding: "8px 14px",
                  borderRadius: 4,
                  border: helpFilterStatus === status ? "1px solid #007bff" : "1px solid #ccc",
                  background: helpFilterStatus === status ? "#007bff" : "#f8f9fa",
                  color: helpFilterStatus === status ? "white" : "black",
                  cursor: "pointer",
                }}
              >
                {status} ({helpStatusCounts[status]})
              </button>
            ))}
          </div>
